# Skill: /daf-remove-global

**When:** The user wants to **uninstall all machine-wide DAF files** installed by **`/daf-onboard`** (global setup).

**Not** per-repo teardown — use **`/daf-remove`** on each project that still has `.agent/`.
...
1. Restate: this removes **`~/.config/agent/`** (or **`DAFE_GLOBAL_ROOT`** if set) and **Cursor `daf-*` skill folders** under `~/.cursor/skills/` (or **`DAF_CURSOR_SKILLS_ROOT`**). Individual repos keep `.agent/` until **`/daf-remove`**.
...
| Target | Contents (installed by `/daf-onboard`) |
|--------|------------------------------------------|
...
Confirm **`$G`** is gone and listed **`daf-*`** folders under **`$CS`** are gone. Tell the user they can run **`/daf-onboard`** again to reinstall.

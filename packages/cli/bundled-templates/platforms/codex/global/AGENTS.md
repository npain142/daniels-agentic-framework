...
2. Installed IDE platforms are in **`~/.config/agent/platforms.json`** (from **`/daf-onboard`**).
3. Run **`/daf-setup`** in a project to copy `.agent/` and merge IDE overlays for all platforms listed there.
...
Repo-root **`AGENTS.md`** in each project should point at `.agent/AGENTS.md` (created by **`/daf-setup`**).

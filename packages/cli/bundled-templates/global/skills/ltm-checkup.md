...
**Not:** In-repo realignment with PRD only — `/daf-realign` in developing/maintaining. Not project setup (`/daf-setup`).
...
1. Load local canon **KG-first** (`.agent/AGENTS.md` §2.1) when `graphify-out/graph.json` exists; always read `memory/remember.md`. If `integrations` includes `"linear"`, use Linear for backlog/history context; otherwise read repo-root `BACKLOG.md` / `todo.txt` and `LOGBACK.md` if present.

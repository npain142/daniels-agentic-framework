...
| Realign intent vs docs in dev | `/daf-realign` |
...
5. End with optional handoffs when relevant: `/daf-help` for framework overview; `/daf-grill-me` (planning) or `/daf-realign` (developing/maintaining); `/daf-how-it-works` for code explanation; `/daf-ltm-checkup` for external memory; `/daf-pivot`; `/daf-improvement`; `/daf-new-feature`; `/daf-backlog-add` to park an idea; `/daf-backlog-work` when ready to build the next backlog item; `/daf-start` when ready to build.

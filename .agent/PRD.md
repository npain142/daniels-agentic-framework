...
- **Version / update:** `daf-pin` + `daf-repo` at onboard; worktree-aware version check (cwd inside DAF monorepo or consumer project worktree); **`daf.mdc`** step 0 runs one-line stale check each session and prompts once to apply **`/daf-update`** per repo.

...
| **Global install runner** | `scripts/global-install.mjs` | Thin CLI invoked by agent during `/onboard`; imports built `packages/cli/dist`. |
| **Onboarding script** | `templates/global/onboarding/global-setup.md` | Agent-readable procedure for global setup (paths, verify, handoff to `/setup`). |
...
| **Templates** | `templates/` | Source of truth; copied to `~/.config/agent/` on `/onboard`. |
...
- **`/onboard`** is machine-wide; **`/setup`** is per-repo (greenfield scaffold or brownfield interview).
